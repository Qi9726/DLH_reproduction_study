...
def generate_dense_encodings(text_corpus, ctx_encoder, out_file):
    ...
    encoder = DPRContextEncoder.from_pretrained(ctx_encoder).to('cuda:0')
    ...
    with open(text_corpus, encoding = "utf-8") as fin:
         ...
         for row in reader:
             ...
             results.extend([(sample_id, embeddings[0,:].detach().cpu().numpy())])
...
def dense_retriever(questions, question_encoder, text_corpus, corpus_embedded, retrieved_passages):
    ...
    model = DPRQuestionEncoder.from_pretrained(question_encoder).to('cuda:0')
    ...
    with open(retrieved_passages, "w", encoding = "utf-8") as fout:
        with open(questions, encoding = "utf-8") as fin:
             ...
             for row in reader:
                 ...
                 questions_embedded.append(embeddings[0,:].detach().cpu())                
...
if __name__ == "__main__":
    ...
    retrieved_passages = "processed/sleep_test_e29_aug.csv"
    span_answers = "processed/pipeline1_label_1.250_aug.csv"
