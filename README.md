...
original paper. In contrast to the retriever, this study was able to achieve the similar level of performance as the SleepQA paper fine-tuned reader model successfully replicated the author’s results, despite using a small batch size of 3. The reason can be that the reader model does not involve contrastive learning, making the batch size less impactful on training performance.  This finding confirms both the reader performance reported in the original paper and the fact that fine-tuning with SleepQA can generate better outcomes compared to the baseline model (BERT SQuAD2).
