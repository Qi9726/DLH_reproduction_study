...
def avg_no_words(train_file):
    ...
    with open( train_file) as f:
        ...
        for example in train_json:
            q = example['question']
            a = example['answers'][0]
            pos_title = example['positive_ctxs'][0]['title']
            pos_passg = example['positive_ctxs'][0]['text']
                
            words_q = word_tokenize(q)
            words_a = word_tokenize(a)
            words_title = word_tokenize(pos_title)
            words_pasasge = word_tokenize(pos_passg)
        
            no_words_q.append(len(words_q))
            no_words_a.append(len(words_a))
            no_words_p.append(len(words_pasasge) 
                              #+ len(words_title)
                              )
...
if __name__ == '__main__':
    avg_no_words('../data/training/sleep-train_aug2.json')